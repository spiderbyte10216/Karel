	...
	public static void checkIfHeadless(String className) {
		if (!JTFTools.testDebugOption("headless")) {
			try {
				...
				Method isHeadless = graphicsEnvironmentClass.getMethod("isHeadless");
				if (!Boolean.TRUE.equals(isHeadless.invoke(null))) return;
			} catch (Exception ex) {
				return;
			}
		}
...
class CommandLineProgramLoader extends ClassLoader {
			...
			for (int index = 1; index < nConstants; index++) {
				...
				switch (type) {
				  case CONSTANT_Integer:
					...
					case CONSTANT_Float:
						JTFTools.copyBytes(in, out, 4); break;
					case CONSTANT_Long:
					case CONSTANT_Double:
						JTFTools.copyBytes(in, out, 8); index++; break;
					case CONSTANT_Class:
					case CONSTANT_String:
						JTFTools.copyBytes(in, out, 2); break;
					case CONSTANT_Utf8:
	...
	private int findSuperclassOffset(InputStream in) {
		HashMap<Integer, Integer> classTable = new HashMap<>();
		try {
			in.skip(8);
			int nConstants = in.read() << 8 | in.read();
			nConstants += 2;
			...
			in.skip(4);
			return classTable.get(in.read() << 8 | in.read());
		} catch (IOException ex) {
			throw new ErrorException(ex);
		}
	}
