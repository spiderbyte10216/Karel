 ...
 */
	public JMenuItem createStandardItem(String action) {
		...
		switch (action) {
			...
			case "Select All" -> item = createFocusedItem(action, 'A');
			case "Save" -> item = createFocusedItem(action, 'S');
			case "Save As" -> item = createFocusedItem(action);
			case "Print" -> {
				item = createProgramItem(action, 'P');
				item.setName("Print...");
			}
			case "Print Console" -> item = createProgramItem(action);
			case "Script" -> {
				item = createProgramItem(action);
				item.setName("Script...");
			}
			...
			default -> throw new ErrorException("Illegal standard menu item: " + action);
		}
		return item;
	}
 ...
 */
	private void setEnabled(JMenu item, String action, boolean flag) {
		int nItems = item.getItemCount();
		for (int i = 0; i < nItems; i++) {
			JMenuItem subItem = item.getItem(i);
...
/* Private instance variables */
	...
	private final ActionListener menuBarListener;
	private ActionListener focusedListener;
	private final HashMap<KeyStroke,JMenuItem> accelerators;
	private final HashSet<JMenuItem> focusedItems;
...
/* Private instance variables */
	private final ProgramMenuBar menuBar;
...
/* Private instance variables */
	private final JCheckBoxMenuItem twin;
}
