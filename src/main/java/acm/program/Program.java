 ...
 */
	public JPanel getRegionPanel(String region) {
		return switch (region) {
			...
			case WEST -> westPanel;
			case EAST -> eastPanel;
			case CENTER -> centerPanel;
			default -> throw new ErrorException("getRegionPanel: Illegal region " + region);
		};
	}
 ...
 */
	public void add(Component comp, String region, Object constraints) {
		switch (region) {
			case NORTH -> northPanel.add(comp, constraints);
			case SOUTH -> southPanel.add(comp, constraints);
			case WEST -> westPanel.add(comp, constraints);
			case EAST -> eastPanel.add(comp, constraints);
			case CENTER -> centerPanel.add(comp, constraints);
			default -> throw new ErrorException("add: Illegal region " + region);
		}
	}
 ...
 */
	public boolean menuAction(ActionEvent e) {
		...
		switch (cmd) {
			...
			case "Export Applet", "Submit Project" -> {
 ...
 */
	protected boolean isStarted() {
		...
		if (console.getParent() == null) return false;
 ...
 */
	private boolean hasActionListener(Component comp) {
		try {
			Method getActionListeners = comp.getClass().getMethod("getActionListeners");
 ...
 */
	private boolean checkForAppletMode() {
		...
		for (int i = 1; i < stack.length; i++) {
			if (stack[i].getMethodName().contains("Applet")) return true;
		}
		return false;
	}
	...
	public void stop() {
		try {
			if (executor == null) {
				...
				stop.invoke(mainThread);
			} else {
				Method shutdownNow = executor.getClass().getMethod("shutdownNow");
				shutdownNow.invoke(executor);
			}
		} catch (Exception ex) {
			/* Empty */
		}
...
class DefaultActionListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (comp instanceof Button) {
			...
			try {
				Method getText = comp.getClass().getMethod("getText");
				message += " to button " + getText.invoke(comp);
			} catch (Exception ex) {
				throw new ErrorException(ex);
			}
		}
