 ...
 */
	private String defaultFormat(double d) {
		...
		if (fm.stringWidth(str.toString()) <= availableSpace) return str.toString();
		int eIndex = str.indexOf("E");
		String suffix = "";
		if (eIndex != -1) {
			suffix = str.substring(eIndex);
			str = new StringBuilder(str.substring(0, eIndex));
			try {
				d = parser.parse(str.toString()).doubleValue();
			} catch (ParseException ex) {
				throw new ErrorException(ex);
			}
		}
		...
		String head = str.substring(0, str.toString().indexOf('.') + 1);
