 ...
 */
	public Dimension getPreferredSize() {
		...
		try {
			Method getNaturalBoundsRect = movieClass.getMethod("getNaturalBoundsRect");
			Method getWidth = qdRectClass.getMethod("getWidth");
			Method getHeight = qdRectClass.getMethod("getHeight");
			Object rect = getNaturalBoundsRect.invoke(movie);
			int width = (Integer) getWidth.invoke(rect, new Object[0]);
			int height = (Integer) getHeight.invoke(rect, new Object[0]);
			return new Dimension(width, height + getControllerHeight());
		} catch (Exception ex) {
 ...
 */
	private void startQuickTime() {
		...
		try {
			...
			Class<?> qtSessionClass = Class.forName("quicktime.QTSession");
			timeRecordClass = Class.forName("quicktime.std.clocks.TimeRecord");
			Method isInitialized = qtSessionClass.getMethod("isInitialized");
			synchronized (lock) {
				if (isInitialized.invoke(null).equals(Boolean.FALSE)) {
					Method open = qtSessionClass.getMethod("open");
					open.invoke(null);
				}
			}
			Method getMajorVersion = qtSessionClass.getMethod("getMajorVersion");
			int version = (Integer) getMajorVersion.invoke(null, new Object[0]);
	...
	private void addQTComponent(Object obj) throws Exception {
		if (!(obj instanceof Component)) {
			...
			if (!(obj instanceof Component)) {
				...
				Method asComponent = qtClass.getMethod("asComponent");
				obj = asComponent.invoke(obj);
			}
		}
		removeAll();
 ...
 */
	private int getCurrentTime() {
		...
		try {
			Method getCurrentTime = movieControllerClass.getMethod("getCurrentTime");
			return (Integer) getCurrentTime.invoke(controller, new Object[0]);
		} catch (Exception ex) {
			throw new ErrorException(ex);
		}
	}
 ...
 */
	private boolean isStopped() {
		...
		try {
			Method getPlayRate = movieControllerClass.getMethod("getPlayRate");
			return (Float) getPlayRate.invoke(controller, new Object[0]) == 0.0;
		} catch (Exception ex) {
			throw new ErrorException(ex);
		}
	}
 ...
 */
	private int getQTDuration() {
		if (!hasQuickTime || movie == null) return 0;
		try {
			Method getDuration = movieClass.getMethod("getDuration");
			return (Integer) getDuration.invoke(movie, new Object[0]);
		} catch (Exception ex) {
			throw new ErrorException(ex);
		}
	}
 ...
 */
	private int getQTTimeScale() {
		...
		try {
			Method getTimeScale = movieClass.getMethod("getTimeScale");
			return (Integer) getTimeScale.invoke(movie, new Object[0]);
		} catch (Exception ex) {
			throw new ErrorException(ex);
		}
	}
 ...
 */
	private Object createTimeRecord(int time) {
		try {
			...
			Object[] args = {getQTTimeScale(), (long) time};
