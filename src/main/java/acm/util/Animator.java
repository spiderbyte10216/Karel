 ...
 */
	public void setSpeed(double speed) {
		animatorSpeed = speed;
		if (speedBar instanceof JSlider slider) {
			int min = slider.getMinimum();
			int max = slider.getMaximum();
			slider.setValue((int) Math.round(min + speed * (max - min)));
		} else if (speedBar instanceof JScrollBar scrollBar) {
 ...
 */
	private void initAnimator() {
		...
		if (applet != null) {
			...
			ArrayList<Thread> list = animatorTable.computeIfAbsent(applet, k -> new ArrayList<Thread>());
			list.add(this);
		}
	}
...
/* Static table of animator threads stored by applet */
	private static final HashMap<Applet,ArrayList<Thread>> animatorTable = new HashMap<>();
 ...
 */
	public static void register(Animator animator, Object speedBar) {
		...
		try {
			listener.getValue = speedBarClass.getMethod("getValue");
			listener.getMinimum = speedBarClass.getMethod("getMinimum");
			listener.getMaximum = speedBarClass.getMethod("getMaximum");
 ...
 */
	public void setSpeed() {
		try {
			int min = (Integer) getMinimum.invoke(speedBar, new Object[0]);
			int max = (Integer) getMaximum.invoke(speedBar, new Object[0]);
			int value = (Integer) getValue.invoke(speedBar, new Object[0]);
